...
import { formatPoemLines, getEffectiveIndent, indentSelectedLines } from "./poemFormatting";
...
describe("poem formatting", () => {
  ...
  it("keeps stanza boundaries and standalone markers", () => {
    const lines = formatPoemLines("첫 줄\n둘째 줄\n＜\n새 연");
  ...
  it("preserves explicit full-width and legacy space indentation", () => {
  ...
  it("applies Tab indentation to every selected line", () => {
    const result = indentSelectedLines("첫 줄\n둘째 줄\n셋째 줄", 0, 8, "in");
